...
class TcpConnection {
   public:
    ...
    // 1回のrecv()で得られた分だけ(最大maxSizeバイト)を返す、いわば
    // 「TCPが実際にどう届くか」をそのまま見せる低レベルAPI。送信側が
    // 1回で送ったデータが、必ずしも1回のrecv()にまとまって届くとは
    // 限らない(複数回に分かれることも、複数メッセージがまとまって
    // 届くこともある)ため、FrameParser::Feed()に渡して境界を
    // 復元するのに使う(38番のメイン教材)。接続が閉じられていれば
    // 空文字列を返す。
    std::string ReceiveSome(size_t maxSize = 4096);
