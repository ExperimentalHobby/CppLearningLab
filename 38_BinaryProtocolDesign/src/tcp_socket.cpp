...
std::string TcpConnection::ReceiveSome(size_t maxSize) {
    if (recvBuffer_.empty()) {
        if (!FillBuffer()) {
            return {};  // 接続が閉じられた
        }
    }
    // windows.h系ヘッダーがmin/maxをマクロとして定義していることがあるため、
    // std::minがマクロ展開されて構文エラーにならないよう括弧で保護する。
    const size_t take = (std::min)(maxSize, recvBuffer_.size());
    const std::string result = recvBuffer_.substr(0, take);
    recvBuffer_.erase(0, take);
    return result;
}
