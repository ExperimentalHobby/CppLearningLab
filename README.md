...
学習リポジトリです。全44課題をルート直下にフラットに配置し、番号帯でカテゴリを表現しています。
...
（例: 09番のマルチスレッド基礎を追加した際も、10番台のGUIプログラミングとは衝突しません）。

| 番号帯 | カテゴリ | 課題数 | 内容 |
|--------|----------|--------|------|
| 01-09 | 基礎 (Fundamentals) | 9 | 環境構築、文法、メモリ管理、STL、OOP、例外、テンプレート、マルチスレッド |
...
| 51-58 | 並行処理 (Concurrency) | 5 | atomic、future/async、スレッドプール、リーダーライタロック、デッドロック、ロックフリー、キャンセル、並列アルゴリズム |
...
| 09 | [09_ConcurrencyBasics](09_ConcurrencyBasics/README.md) | std::thread/mutex/condition_variableによるマルチスレッド基礎 |
...
5. **41-47 USB** で物理デバイスとの連携に触れる（実機やUSBデバイスがない場合は仮想COMポートや
   エミュレータでの代替も検討）
6. **51-58 並行処理** で09番のマルチスレッド基礎を発展させ、atomic・非同期処理・
   スレッドプール・ロックフリーアルゴリズムなど、より実践的な並行処理を学ぶ
